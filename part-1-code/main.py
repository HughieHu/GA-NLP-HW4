...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    with torch.no_grad():
        ...
        initial_logits = test_outputs.logits
        print(f"Initial Logits: {initial_logits.cpu().numpy()}")
        initial_probs = torch.softmax(initial_logits, dim=-1)
        print(f"Initial Probabilities: {initial_probs.cpu().numpy()}")
    print("=" * 80 + "\n")
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if torch.isnan(loss):
                print(f"\nWarning: Loss is NaN at Epoch {epoch+1}, Batch {batch_idx}")
                print("Stopping training to prevent further model collapse")
                return
            ...
            if batch_idx % 100 == 0 and batch_idx > 0:
                recent_grad_norm = sum(grad_norms[-100:]) / min(100, len(grad_norms))
                if recent_grad_norm > 10.0:
                    print(f"\nWarning: Large gradient! Average gradient norm: {recent_grad_norm:.2f}")
        ...
        with torch.no_grad():
            test_outputs = model(**test_inputs)
            epoch_logits = test_outputs.logits
            print(f"Epoch {epoch + 1} Logits: {epoch_logits.cpu().numpy()}")
            epoch_probs = torch.softmax(epoch_logits, dim=-1)
            print(f"Epoch {epoch + 1} Probabilities: {epoch_probs.cpu().numpy()}")
        model.train()
    ...
    if len(epoch_losses) >= 2:
        ...
        if epoch_losses[-1] > epoch_losses[0] * 0.9:
            ...
            print(f"   From {epoch_losses[0]:.4f} to {epoch_losses[-1]:.4f}")
            print(f"   Reduction: {loss_reduction:.2f}%")
