...
def create_prompt(sentence, k):
    ...
    if ptype == 0:
        ...
        instruction = "You are a SQL expert. Convert the following natural language question to a SQL query.\n\n"
        if schema:
            instruction += f"Database Schema:\n{schema}\n\n"
    elif ptype == 2:
        instruction = """You are a SQL expert. Your task is to convert natural language questions into SQL queries."""
...
def eval_outputs(eval_x, eval_y, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    if gt_sql_path is None or gt_record_path is None:
        print(f"Queries saved to {model_sql_path}")
    ...
    error_rate = error_count / len(model_error_msgs) if len(model_error_msgs) > 0 else 0.0
